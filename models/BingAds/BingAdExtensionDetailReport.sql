...
{% endif %}
    ...
    From (
        ...
        ROW_NUMBER() OVER (PARTITION BY  AccountName,AdExtensionId,adGroupId,AdId,CampaignId,DeliveredMatchType,Network,DeviceOS,DeviceType,date(TimePeriod) order by {{daton_batch_runtime()}} desc) row_num
        from {{i}}	
            {% if is_incremental() %}
