...
{% set result =set_table_name("BingAccountPerformanceReport_tbl_ptrn","BingAccountPerformanceReport_tbl_exclude_ptrn") %}
...
{% for i in result %}
    ...
    from {{i}} a  
        ...
        {% if is_incremental() %}
            ...
            where a.{{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('BingAccountPerformanceReport_lookback') }},0) from {{ this }})
            --WHERE 1=1
        {% endif %}
