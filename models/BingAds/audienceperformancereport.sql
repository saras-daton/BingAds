{% if var('audienceperformancereport') %}
    {{ config( enabled = True ) }}
{% else %}
...
{% endif %}
    ...
    {{set_table_name('%audience_performance_report')}}    
    {% endset %}  
    ...
    From (
        ...
        ROW_NUMBER() OVER (PARTITION BY adGroupId,campaignId,AudienceId order by TimePeriod desc) row_num
        from {{i}}	
            {% if is_incremental() %}
