...
{% for i in relations %}
    ...
    {% endif %}
        ...
        from {{i}} a  
           ...
           where {{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('BingKeywordPerformanceReport_lookback') }},0) from {{ this }})
            --WHERE 1=1
            {% endif %}
        Qualify ROW_NUMBER() OVER (PARTITION BY CampaignId,adGroupId,KeywordId,AdId,DeliveredMatchType,BidMatchType,DeviceOS,TimePeriod order by {{daton_batch_runtime()}} desc) = 1
